...
def align_tokens(ref_tokens: list[str], hyp_tokens: list[str]) -> list[tuple[str, str, float]]:
...
def expert_alignment(ref: str, hyp: str) -> dict:
    ...
    text = sum((1 for item_r, item_h, text in pairs if item_r and item_h and (text < 1.0)))
    ...
    for item_r, item_h, text in pairs:
        if item_r and item_h:
            if text == 1.0:
                ...
                chunks.append(f"<span title='эталон: {item_r}; похожесть: {text:.2f}' style='background:#fff3cd;border-bottom:1px dotted #f0ad4e'>{item_h}</span>")
        elif item_r and (not item_h):
            chunks.append(f"<span title='удалено; эталон: {item_r}' style='background:#ffe0e0;text-decoration:line-through'>{item_r}</span>")
        elif item_h and (not item_r):
            chunks.append(f"<span title='вставка' style='background:#e0f0ff'>{item_h}</span>")
    diff_html = ' '.join(chunks)
    return {'alignment': [{'ref': item_r, 'hyp': item_h, 'sim': round(text, 3)} for item_r, item_h, text in pairs], 'wer': wer, 'char_sim': char_sim, 'diff_html': diff_html}
